...
class TestUpdateWorkout:
    ...
    @pytest.mark.asyncio
    async def test_update_duration_zero_clears_planned_time(self):
        """duration_minutes=0 should null totalTimePlanned (empty Duration in UI)."""
        existing = {
            "workoutId": 1001,
            "title": "ДО",
            "totalTimePlanned": 0.0167,
        }
        get_response = APIResponse(success=True, data=existing)
        put_response = APIResponse(success=True, data=None)
        ...
        assert result["success"] is True
        put_payload = mock_instance.put.call_args[1]["json"]
        assert put_payload["totalTimePlanned"] is None
